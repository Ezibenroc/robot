...
mainRobot(State,[X,Y],EntryPoints,ExitPoints,Termination,TerminationRequester,ID) ->
    receive
        {PID,terminate_request} -> mainRobot(State,[X,Y],EntryPoints,ExitPoints,true,[PID|TerminationRequester],ID)
    after ?TIME_REC ->
        if
            Termination -> robotUtils:multiSend(TerminationRequester,{self(),ackTerminate}), terminated;
            true -> noMessage(State,[X,Y],EntryPoints,ExitPoints,Termination,TerminationRequester,ID)
        end
    end.

% Function called when there is no message to read, and process do not need to terminate.
noMessage(State,[X,Y],EntryPoints,ExitPoints,Termination,TerminationRequester,ID) ->
    mainRobot(State,[X,Y],EntryPoints,ExitPoints,Termination,TerminationRequester,ID).
